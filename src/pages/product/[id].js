...
export default function ProductPage({ product }) {
  ...
  const images = product.images?.length > 0 ? product.images : [null, null, null];
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#0a0a0a]' : 'bg-[#f5f5f0]'}`}>
      ...
      {/* Main Image */}
      <div className={`relative aspect-square mx-4 rounded-2xl overflow-hidden ${isDark ? 'bg-[#141414]' : 'bg-white'}`}>
        {images[activeImg] ? (
      ...
      <div className="px-4 mt-5 pb-32">
        ...
        <h1 className={`font-serif text-xl font-bold mt-1 mb-3 leading-snug ${isDark ? 'text-white' : 'text-gray-900'}`}>
          {product.name}
        </h1>
        ...
        <div className={`flex items-center gap-3 p-4 rounded-xl mb-4 ${isDark ? 'bg-[#141414]' : 'bg-white'}`}>
          <div>
            ...
            <p className="text-emerald-500 font-bold text-2xl">৳{Number(product.discount_price).toLocaleString()}</p>
      ...
      {product.available && (
        <div className={`fixed bottom-16 left-0 right-0 px-4 pb-3 pt-2 ${isDark ? 'bg-[#0a0a0a]/95 border-t border-[#1f1f1f]' : 'bg-white/95 border-t border-gray-100'} backdrop-blur-sm`}>
